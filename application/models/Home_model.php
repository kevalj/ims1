...
class Home_model extends CI_Model {
		...
		{
			$condition = " id=".$this->db->escape($this->session->userdata['logged_in']['userid'])." and status='Y'";
			$this->db->select('*');
			$this->db->from('user_login_details');
			$this->db->where($condition);
			$query = $this->db->get();
		...
		public function get_user_depot_data($userid)
		{
			$query="select group_concat(depotId) depotId from user_depot_relation where status='Y' and user_id=".$this->db->escape($userid)."";
			 $query = $this->db->query($query);
			return $query->result_array();
		}
